...
const ALL_TYPES: EventType[] = ['RecordWritten', 'ConsentGranted', 'ConsentRevoked', 'RecordAccessed']
    ...
    <div className="min-h-screen bg-obsidian">
      ...
      <header className="border-b border-bone/10 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-bone/40 text-sm">/ Audit Trail</span>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => setRefreshKey(k => k + 1)}
            className="flex items-center gap-1.5 text-xs text-bone/50 hover:text-bone transition-colors"
          >
      ...
      <div className="bg-accent/10 border-b border-accent/20 px-6 py-2 text-sm text-accent/90 flex items-center gap-2">
        ...
        Demo mode — events simulated. In production, events are polled from Polygon Amoy via contract logs.
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {ALL_TYPES.map((type) => {
            ...
            return (
              ...
              >
                <div className={`flex items-center gap-1.5 mb-1 ${filter === type ? cfg.color : 'text-bone/60'}`}>
                  <Icon className="w-3.5 h-3.5" />
                  <span className="text-xs font-medium">{cfg.label}</span>
                </div>
                <p className={`text-xl font-bold ${filter === type ? cfg.color : 'text-bone'}`}>{count}</p>
              </button>
            )
          })}
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-1">
                      ...
                      <span className="text-xs text-bone/40 shrink-0">{timeAgo(event.timestamp)}</span>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-0.5">
                      ...
                      <p className="text-xs text-bone/50">
                        Block: <span className="font-mono text-bone/70">{event.blockNumber.toLocaleString()}</span>
                      </p>
                      {event.recordId && (
                        <p className="text-xs text-bone/50">
                          Record: <span className="font-mono text-bone/70">{short(event.recordId)}</span>
                        </p>
                      )}
                      {event.extra && (
                        <p className="text-xs text-bone/50">{event.extra}</p>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mt-1.5">
                      <p className="font-mono text-xs text-bone/40 truncate">{short(event.txHash)}</p>
                      <a
                        href={`https://amoy.polygonscan.com/tx/${event.txHash}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="shrink-0 text-bone/30 hover:text-accent transition-colors"
